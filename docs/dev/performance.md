...
`maximal --curve <index.tsv>` walks a run's accumulated candidates in timestamp order, keeping the maximal antichain under implication, and prints an event log of admissions, drops and removals. `score_curves.py --maximality` replays every time cut's membership from that one log, where it used to run a batch `maximal` over each cut's prefix, re-deciding pairs the previous cut had decided with a cold solver cache each time. Each arrival meets the running antichain alone, so the walk is O(n * |antichain|) rather than O(n * |prefix|).

The walk (`antichain::running_antichain`, `src/filter/running_antichain.hpp`) runs in waves. Each wave scans its arrivals concurrently against a snapshot of the antichain taken before the wave, stopping at the first dominating member. A stale snapshot is sound because implication is transitive: a candidate dominated by `a` stays dominated by whatever removed `a`, and a member removed by `x` stays dominated by whatever dropped `x`. What a snapshot misses is two arrivals of one wave that are comparable to each other, so a second parallel region settles the wave's survivors pairwise, and a serial replay in discovery order applies the wave with no further solver call. The log is therefore a function of the arrivals and not of the wave size, which `test_wave_size_does_not_change_the_log` crosses at 1, 2, 3 and 16. Both directions go through the sampled-word prefilter first, and an equivalence class collapses on `prefer_first` as in the batch sweep, so the walk's final membership is the batch filter's survivors wherever no check times out.
