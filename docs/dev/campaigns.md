...
The two passes read differently shaped trees, and the shape is checked rather than assumed. The anytime pass reads the raw tree `aurus_campaign.py` writes, `<results>/<spec>/repeat-<NN>`, and it has to be that one: a solution is dated from its repeat's own record of when it was found, and `aurus_adapt.py` copies the dates it derived into its tree but neither record they came from. Which record dated a row is the `dated_by` column. The fork at `e1cfadf` appends each solution's discovery time to `solution-times.csv` as it writes the solution, to the microsecond, and that file is preferred wherever it is present; upstream AuRUS writes every solution in one batch at the end and leaves only the `run.log`, whose per-iteration `#Sol` column dates solution *i* at the first iteration reaching *i*+1, to the second. One tree may therefore hold repeats of both vintages, and the column says which is which rather than leaving a reader to assume the resolution. That distinction is what the 2026-09-22 re-run exists to record: scoring the fork's arm with the log reader would round its times to the second and say nothing about having done so. The wellsep pass reads the PEREDUR-shaped tree `aurus_adapt.py` leaves, `<results>/<run>_seed<NN>`, and is pointed at each run's `accumulated/` rather than at the run, so nothing beside the candidates can add a row. A phase pointed at the wrong one is refused by name; the alternative reading is an empty queue, which is what a host whose search has not run yet looks like.
