...
TEST(test_tlsf_subsumption_agrees_with_ltlsynt) {
...
TEST(test_spec_implies_reflexive) {
...
TEST(test_spec_implies_weakening_direction) {
    SatisfiabilityChecker& checker = global_sat_checker();
...
TEST(test_unsatisfiable_assumptions_detected) {
    SatisfiabilityChecker& checker = global_sat_checker();
...
TEST(test_assumption_side_stays_a_joint_query) {
    SatisfiabilityChecker& checker = global_sat_checker();
...
TEST(test_implication_filter_keeps_maximal) {
    SatisfiabilityChecker& checker = global_sat_checker();
    const tlsf::Specification base = base_spec();
    const tlsf::Specification weaker = weaker_spec();
...
TEST(test_well_separation_drops_output_liveness_assumption) {
...
TEST(test_well_separation_keeps_reactive_output_assumption) {
    RealizabilityChecker checker;
    const FilterFunctionT<tlsf::Specification> filter =
        make_well_separation_filter<tlsf::Specification>(checker);
...
TEST(test_well_separation_keeps_input_only_assumption) {
    RealizabilityChecker checker;
    const FilterFunctionT<tlsf::Specification> filter =
        make_well_separation_filter<tlsf::Specification>(checker);
