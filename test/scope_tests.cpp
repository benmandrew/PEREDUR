...
constexpr std::string_view k_test_suite = "scope";
...
TEST(test_global_scope_lowering_is_unchanged) {
    struct Row {
...
TEST(test_global_scope_fretish_is_unchanged) {
...
TEST(test_scope_zero_ticks_is_unrelaxed) {
...
TEST(test_scope_is_part_of_requirement_identity) {
...
TEST(test_atom_prefix_tags_the_mode) {
...
TEST(test_environment_signals_appends_modes) {
...
// expands far larger.
TEST(test_scope_agrees_with_formaliser) {
...
TEST(test_condition_type_order_is_pinned) {
    for (const ScopeCase& scope_case : scope_cases()) {
        for (const Timing& tim : timing_cases()) {
...
TEST(test_scope_order_is_pinned) {
...
// between them.
TEST(test_stop_timing_order_is_pinned) {
